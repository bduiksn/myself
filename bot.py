...
async def send_phone_request(user_id: int):
    """Require the user's own Iranian phone number via Telegram contact sharing."""
    text = "📱 برای ادامه فعالیت در ربات، لطفاً شماره تلفن خودتون رو به اشتراک بذارید.\n\nفقط شماره ایران (+98) پذیرفته می‌شود."
    try:
        button = Button.request_phone("📱 اشتراک‌گذاری شماره", style="success")
    except Exception:
        button = Button.request_phone("📱 اشتراک‌گذاری شماره")
    await bot.send_message(user_id, text, buttons=[[button]])
...
from self_worker import start_self_worker, stop_self_worker
...
# Self account message handling is implemented in self_worker.py.
...
async def private_message(event):
    ...
    if event.contact:
        ...
        with contextlib.suppress(Exception):
            await bot.send_message(user_id, "✅ شماره ایران شما با موفقیت ثبت شد.", buttons=Button.clear())
        await send_main(user_id, user_id)
        return
